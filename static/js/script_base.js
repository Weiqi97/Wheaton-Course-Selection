...
// Function to convert time to a proper format.
function timeConverter(time) {
    if (time.includes("A")) {
        ...
        if (Number(time.slice(0, 2)) !== 12) {
            const number = Number(time.slice(0, 1)) + 12;
            return String(number) + time.slice(1, 4) + ":00";
        }
        else
            return time.slice(0, 5) + ":00";
    }
...
// Function to convert day to a proper format.
...
$(function () {
    ...
    data_table.find("tbody").click("#show_detail", function (e) {
    ...
    // Gets data from the row clicked and insert it into the calendar.
